...
const CodeBlockComponent = ({ command }: CodeBlockProps) => {
  ...
  return (
    <div className="bg-[#0c0c0e] rounded-lg overflow-hidden flex items-center justify-between group">
      <pre className="p-4 text-sm text-zinc-300 overflow-x-auto flex-1">
        <code>{command}</code>
      </pre>
      <button
        onClick={handleCopy}
